...
def estimate_monte_carlo_returns_trifinger(env,
                                 discount,
                                 actor,
                                 std,
                                 num_episodes,
  ...
  for i in pbar:
    ...
    actor.reset()
    t = 0
    observation = inital_obs
    # single episode
    
    while True:
      _, action, _ = actor(observation, std)
      observation, rew, _, truncated, info = env.step(action)
      # rew = behavior_dataset.normalize_rewards([rew])[0]
...
class TrifingerActor(object):
  ...
  def __call__(self, obs, std= 0.05, actions=None):
      ...
      if actions is None:
          ...
          normal = torch.distributions.Normal(action_mean, self.std)
          #action = normal.sample()
          actions = torch.clamp(actions,-0.397,  0.397)
